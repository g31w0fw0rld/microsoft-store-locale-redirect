...
- **The platform tag is stripped from the name** — this store returns titles like "Roblox - Windows", which neither destination uses. **GG.deals opens already filtered to the Microsoft Store DRM** and with no store-rating floor; it gets the full title. PCGamingWiki gets **the game itself**: without the edition suffix and, on a DLC or an edition, by its base game, taken from the product group Microsoft shares between every SKU of one game — and used only when it really appears inside the title, because sometimes it is an internal name (`Boost` for *Devil May Cry 5 Special Edition*) and searching for that would be worse than doing nothing.
...
- **Al nombre se le quita la coletilla de plataforma** —esta tienda devuelve títulos como "Roblox - Windows", que ninguno de los dos destinos usa—. **GG.deals se abre ya filtrado por el DRM de Microsoft Store** y sin el mínimo de valoración de tienda; recibe el título completo. PCGamingWiki recibe **el juego en sí**: sin el sufijo de edición y, en un DLC o una edición, por su juego base, sacado del grupo de producto que Microsoft comparte entre todos los SKU de un mismo juego —y usado solo si de verdad aparece dentro del título, porque a veces es un nombre interno (`Boost` en *Devil May Cry 5 Special Edition*) y buscar eso sería peor que no tocar nada—.
